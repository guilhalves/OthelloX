...
void CreateWindow(BOARD *board, int starter_player, int daltonism, int player_vs_player) {
	...
	while (running)
	{
		...
		if (!GameFinished(*board))
		{
			...
			if (!(player_vs_player))
			{
				...
				// (Note que na função main, starter_player=!starter_player);

				if ((*board).turn == starter_player)
				{
					eval_history[num_disks-4]=-EvalFunction(*board, 1);
					...
					cell_board=SearchBest(*board, initial_time, zobrist_table, hash_table, 2);
				...
				else
				{
					...
					//cell_board=CoordToBit(coluna-1, linha-1);
		...
		{
			dif=DiskCount((*board).playerBoards[BLACK])-DiskCount((*board).playerBoards[WHITE]);
			if (dif < 0)
			{
				white_wins++;
			} else if (dif > 0)
			...
			played_matchs++;
			printf("%d%%\n", played_matchs);
			printf("PRETO: %d, BRANCO: %d\n", DiskCount((*board).playerBoards[BLACK]), DiskCount((*board).playerBoards[WHITE]));
			
			printf("BLACK WINS: %d\nWHITE WINS: %d\nDRAWS: %d\n\n", black_wins, white_wins, draws);
			printf("TEMPORARY WIN RATE: %lf\n", black_wins/(1.0*played_matchs));
			ResetBoard(board);
	...
	eval_history[num_disks-4]=DiskCount((*board).playerBoards[BLACK])-DiskCount((*board).playerBoards[WHITE]);
